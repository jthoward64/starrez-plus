...
export class EntryMeal {
  ...
  static async fetchById(id: number, starRezConfig: StarRezRestConfig): Promise<EntryMeal | null> {
    const fetchUrl = new URL(starRezConfig.baseUrl);
    fetchUrl.pathname = `${fetchUrl.pathname}/services/select/EntryMeal/${id}`;
    const response = await doStarRezRequest(fetchUrl, starRezConfig);

    if (!response.ok) {
      throw new Error(`Failed to fetch EntryMeal with id ${id}`);
    } else {
