...
export class ConcernNote {
  ...
  static async fetchById(id: number, starRezConfig: StarRezRestConfig): Promise<ConcernNote | null> {
    const fetchUrl = new URL(starRezConfig.baseUrl);
    fetchUrl.pathname = `${fetchUrl.pathname}/services/select/ConcernNote/${id}`;
    const response = await doStarRezRequest(fetchUrl, starRezConfig);

    if (!response.ok) {
      throw new Error(`Failed to fetch ConcernNote with id ${id}`);
    } else {
