...
export class RoomLocation {
  ...
  static async fetchById(id: number, starRezConfig: StarRezRestConfig): Promise<RoomLocation | null> {
    const fetchUrl = new URL(starRezConfig.baseUrl);
    fetchUrl.pathname = `${fetchUrl.pathname}/services/select/RoomLocation/${id}`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch RoomLocation with id ${id}`);
    } else {
