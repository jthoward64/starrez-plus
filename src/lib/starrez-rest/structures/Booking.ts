...
export class Booking {
  ...
  /**
   * Fetches a Booking by its ID or by exact match on other fields.
   * @param param Either the ID of the Booking to fetch, or an object of key-value pairs to match against.
   * @param starRezConfig The configuration to use for the request.
   * @returns A promise that resolves to a single Booking object or null (if id) or an array of Booking objects (if other fields).
   */
  // overrides
  static async select(param: number, starRezConfig: StarRezRestConfig): Promise<Booking | null>;
  static async select(param: Partial<Record<keyof Booking, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<Booking[]>;
  static async select(param: number | Partial<Record<keyof Booking, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<Booking | Booking[] | null> {
    const fetchUrl = new URL(starRezConfig.baseUrl);
    if (typeof param === 'number') {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/Booking/${param}`;
    } else {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/Booking`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch Booking with param ${JSON.stringify(param)}`);
    } else {
      if (typeof param === 'number') {
        ...
        return entries.map(entry => new Booking(entry));
      }
    }
  }
}
