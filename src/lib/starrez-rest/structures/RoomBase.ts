...
export class RoomBase {
  ...
  /**
   * Fetches a RoomBase by its ID or by exact match on other fields.
   * @param param Either the ID of the RoomBase to fetch, or an object of key-value pairs to match against.
   * @param starRezConfig The configuration to use for the request.
   * @returns A promise that resolves to a single RoomBase object or null (if id) or an array of RoomBase objects (if other fields).
   */
  // overrides
  static async select(param: number, starRezConfig: StarRezRestConfig): Promise<RoomBase | null>;
  static async select(param: Partial<Record<keyof RoomBase, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<RoomBase[]>;
  static async select(param: number | Partial<Record<keyof RoomBase, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<RoomBase | RoomBase[] | null> {
    const fetchUrl = new URL(starRezConfig.baseUrl);
    if (typeof param === 'number') {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/RoomBase/${param}`;
    } else {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/RoomBase`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch RoomBase with param ${JSON.stringify(param)}`);
    } else {
      if (typeof param === 'number') {
        ...
        return entries.map(entry => new RoomBase(entry));
      }
    }
  }
}
