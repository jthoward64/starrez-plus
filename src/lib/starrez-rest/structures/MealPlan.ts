...
export class MealPlan {
  ...
  /**
   * Fetches a MealPlan by its ID or by exact match on other fields.
   * @param param Either the ID of the MealPlan to fetch, or an object of key-value pairs to match against.
   * @param starRezConfig The configuration to use for the request.
   * @returns A promise that resolves to a single MealPlan object or null (if id) or an array of MealPlan objects (if other fields).
   */
  // overrides
  static async select(param: number, starRezConfig: StarRezRestConfig): Promise<MealPlan | null>;
  static async select(param: Partial<Record<keyof MealPlan, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<MealPlan[]>;
  static async select(param: number | Partial<Record<keyof MealPlan, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<MealPlan | MealPlan[] | null> {
    const fetchUrl = new URL(starRezConfig.baseUrl);
    if (typeof param === 'number') {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/MealPlan/${param}`;
    } else {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/MealPlan`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch MealPlan with param ${JSON.stringify(param)}`);
    } else {
      if (typeof param === 'number') {
        ...
        return entries.map(entry => new MealPlan(entry));
      }
    }
  }
}
