...
export class DeletedTransaction {
  ...
  /**
   * Fetches a DeletedTransaction by its ID or by exact match on other fields.
   * @param param Either the ID of the DeletedTransaction to fetch, or an object of key-value pairs to match against.
   * @param starRezConfig The configuration to use for the request.
   * @returns A promise that resolves to a single DeletedTransaction object or null (if id) or an array of DeletedTransaction objects (if other fields).
   */
  // overrides
  static async select(param: number, starRezConfig: StarRezRestConfig): Promise<DeletedTransaction | null>;
  static async select(param: Partial<Record<keyof DeletedTransaction, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<DeletedTransaction[]>;
  static async select(param: number | Partial<Record<keyof DeletedTransaction, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<DeletedTransaction | DeletedTransaction[] | null> {
    const fetchUrl = new URL(starRezConfig.baseUrl);
    if (typeof param === 'number') {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/DeletedTransaction/${param}`;
    } else {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/DeletedTransaction`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch DeletedTransaction with param ${JSON.stringify(param)}`);
    } else {
      if (typeof param === 'number') {
        ...
        return entries.map(entry => new DeletedTransaction(entry));
      }
    }
  }
}
