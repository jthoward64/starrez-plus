...
export class ChargeGroup {
  ...
  /**
   * Fetches a ChargeGroup by its ID or by exact match on other fields.
   * @param param Either the ID of the ChargeGroup to fetch, or an object of key-value pairs to match against.
   * @param starRezConfig The configuration to use for the request.
   * @returns A promise that resolves to a single ChargeGroup object or null (if id) or an array of ChargeGroup objects (if other fields).
   */
  // overrides
  static async select(param: number, starRezConfig: StarRezRestConfig): Promise<ChargeGroup | null>;
  static async select(param: Partial<Record<keyof ChargeGroup, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<ChargeGroup[]>;
  static async select(param: number | Partial<Record<keyof ChargeGroup, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<ChargeGroup | ChargeGroup[] | null> {
    const fetchUrl = new URL(starRezConfig.baseUrl);
    if (typeof param === 'number') {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/ChargeGroup/${param}`;
    } else {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/ChargeGroup`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch ChargeGroup with param ${JSON.stringify(param)}`);
    } else {
      if (typeof param === 'number') {
        ...
        return entries.map(entry => new ChargeGroup(entry));
      }
    }
  }
}
