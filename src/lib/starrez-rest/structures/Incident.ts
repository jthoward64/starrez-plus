...
export class Incident {
  ...
  /**
   * Fetches a Incident by its ID or by exact match on other fields.
   * @param param Either the ID of the Incident to fetch, or an object of key-value pairs to match against.
   * @param starRezConfig The configuration to use for the request.
   * @returns A promise that resolves to a single Incident object or null (if id) or an array of Incident objects (if other fields).
   */
  // overrides
  static async select(param: number, starRezConfig: StarRezRestConfig): Promise<Incident | null>;
  static async select(param: Partial<Record<keyof Incident, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<Incident[]>;
  static async select(param: number | Partial<Record<keyof Incident, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<Incident | Incident[] | null> {
    const fetchUrl = new URL(starRezConfig.baseUrl);
    if (typeof param === 'number') {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/Incident/${param}`;
    } else {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/Incident`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch Incident with param ${JSON.stringify(param)}`);
    } else {
      if (typeof param === 'number') {
        ...
        return entries.map(entry => new Incident(entry));
      }
    }
  }
}
