...
export class Group {
  ...
  /**
   * Fetches a Group by its ID or by exact match on other fields.
   * @param param Either the ID of the Group to fetch, or an object of key-value pairs to match against.
   * @param starRezConfig The configuration to use for the request.
   * @returns A promise that resolves to a single Group object or null (if id) or an array of Group objects (if other fields).
   */
  // overrides
  static async select(param: number, starRezConfig: StarRezRestConfig): Promise<Group | null>;
  static async select(param: Partial<Record<keyof Group, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<Group[]>;
  static async select(param: number | Partial<Record<keyof Group, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<Group | Group[] | null> {
    const fetchUrl = new URL(starRezConfig.baseUrl);
    if (typeof param === 'number') {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/Group/${param}`;
    } else {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/Group`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch Group with param ${JSON.stringify(param)}`);
    } else {
      if (typeof param === 'number') {
        ...
        return entries.map(entry => new Group(entry));
      }
    }
  }
}
