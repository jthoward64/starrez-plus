...
export class Event {
  ...
  /**
   * Fetches a Event by its ID or by exact match on other fields.
   * @param param Either the ID of the Event to fetch, or an object of key-value pairs to match against.
   * @param starRezConfig The configuration to use for the request.
   * @returns A promise that resolves to a single Event object or null (if id) or an array of Event objects (if other fields).
   */
  // overrides
  static async select(param: number, starRezConfig: StarRezRestConfig): Promise<Event | null>;
  static async select(param: Partial<Record<keyof Event, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<Event[]>;
  static async select(param: number | Partial<Record<keyof Event, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<Event | Event[] | null> {
    const fetchUrl = new URL(starRezConfig.baseUrl);
    if (typeof param === 'number') {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/Event/${param}`;
    } else {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/Event`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch Event with param ${JSON.stringify(param)}`);
    } else {
      if (typeof param === 'number') {
        ...
        return entries.map(entry => new Event(entry));
      }
    }
  }
}
