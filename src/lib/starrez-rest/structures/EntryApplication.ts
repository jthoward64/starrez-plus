...
export class EntryApplication {
  ...
  /**
   * Fetches a EntryApplication by its ID or by exact match on other fields.
   * @param param Either the ID of the EntryApplication to fetch, or an object of key-value pairs to match against.
   * @param starRezConfig The configuration to use for the request.
   * @returns A promise that resolves to a single EntryApplication object or null (if id) or an array of EntryApplication objects (if other fields).
   */
  // overrides
  static async select(param: number, starRezConfig: StarRezRestConfig): Promise<EntryApplication | null>;
  static async select(param: Partial<Record<keyof EntryApplication, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<EntryApplication[]>;
  static async select(param: number | Partial<Record<keyof EntryApplication, {toString: () => string}>>, starRezConfig: StarRezRestConfig): Promise<EntryApplication | EntryApplication[] | null> {
    const fetchUrl = new URL(starRezConfig.baseUrl);
    if (typeof param === 'number') {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/EntryApplication/${param}`;
    } else {
      fetchUrl.pathname = `${fetchUrl.pathname}/services/select/EntryApplication`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch EntryApplication with param ${JSON.stringify(param)}`);
    } else {
      if (typeof param === 'number') {
        ...
        return entries.map(entry => new EntryApplication(entry));
      }
    }
  }
}
